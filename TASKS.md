The user has pointed out a deprecation warning related to the `useFormState` hook. The new recommended hook is `useActionState` from React.

My plan is to:
1.  Add a new task to `TASKS.md` to address this deprecation warning.
2.  Update the `PublicCalendar` component located at `src/components/trainer/PublicCalendar.tsx`.
3.  I will change the import from `react-dom` to import `useActionState` from `react` instead.
4.  I will then rename the `useFormState` hook call to `useActionState`. The hook's signature is the same, so no other changes are needed for it to function correctly.
5.  Mark the new task as complete in `TASKS.md`.
