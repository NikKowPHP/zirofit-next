...
import {
  ...
  type Exercise,
} from "@/app/[locale]/client-dashboard/actions";

interface UseMyExerciseLogManagerProps {
  initialExerciseLogs: ClientExerciseLog[];
}

const initialActionState: ExerciseLogFormState = { success: false };

export const useMyExerciseLogManager = ({
  initialExerciseLogs,
}: UseMyExerciseLogManagerProps) => {
  ...
  const handleDelete = async (logId: string) => {
    ...
    try {
      const result = await deleteMyExerciseLog(logId);
      if (!result?.success) {
