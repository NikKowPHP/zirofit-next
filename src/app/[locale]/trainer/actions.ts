...
const getContactFormSchema = async () => {
  const locale = await getLocale();
  const t = await getTranslations({ locale, namespace: "ZodErrors" });
  
  return z.object({
    name: z.string().min(1, { message: t('contact_name_required') }),
    email: z.string().email({ message: t('invalid_email') }),
    message: z
      .string()
      .min(10, { message: t('contact_message_min_10') }),
...
export async function submitContactForm(
  ...
  const contactFormSchema = await getContactFormSchema();
