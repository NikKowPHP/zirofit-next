...
export default function ClientForm({ initialData }: ClientFormProps) {
  const [state, dispatch] = useFormState<State, FormData>(
    async (prevState: State, formData: FormData): Promise<State> => {
      const result = await addClient(prevState, formData);
      return {
  ...
  return (
    <form action={dispatch} className="space-y-4">
      ...
      <div>
        ...
        <Input type="text" id="phone" name="phone" defaultValue={initialData?.phone || ''} required />
        {state?.errors?.phone && <p className="text-red-500 text-sm">{state?.errors?.phone[0]}</p>}
      </div>
      <div>
        <Label htmlFor="status">Status</Label>
        <select id="status" name="status" defaultValue={initialData?.status || 'pending'} className="w-full p-2 border rounded">
